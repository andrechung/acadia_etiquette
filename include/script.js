...
(function( $ ) {
 $.fn.populateStickerBarcode = function(fieldPrefix, value) { 
	...
	var barcodeSettings = {
		addQuietZone: true,
		barWidth: 2, barHeight: barHeight, // TODO : rendre taille d'affichage r�elle g�rable par CSS ?
		color: "black",
		bgColor: "transparent",
		showHRI: false, // HRI du plugin inutilis�, remplac� par version custom
...
var AmzContainers = {
	...
	extractItemRows: function(rows){
		...
		if (rows[0].length < 7){
			/* check type fichier : test nombre colonnes, sur 1�re ligne seulement) */
			GuiMessage.error("Fichier A attendu avec 7 colonnes min. (seulement " + rows[0].length + " trouv�s)");
			return -1;
		}
		...
		var groupsByContainer = {}; // obj. "tableau associatif" des �tiquettes, par container_id (= nombre de suivi du carton). Attention, ce dernier doit �tre un attribut obj JS valide !
		for (var i=1; i<rows.length; i++) {// skip first  (pour "PapaParse::header: false")
	
			/* 1.a) Mapping fichier CSV */
			var order_id     = rows[i][0]; // Num�ro de commande
			var container_id = rows[i][5]; // Num�ro de suivi du carton
			var item_id      = "" +rows[i][1]; // UPC/EAN/ISBN
			var item_desc    = rows[i][4]; // Description de l'article
			var item_count   = rows[i][6]; // Quantit� totale de l'exp�dition
			var item_asin    = EanList[item_id]; // Asin de l'ean
			
			if (item_asin != rows[i][3]) {
				if ( item_asin == undefined) {
					GuiMessage.warn("Fichier A, Ligne " + i +" EAN(A):" + item_id + " n'est pas trouv� dans la list ean-amazon"); 
				} else {
			...
			if (!isNaN(item_count)) {
				...
				GuiMessage.warn("Fichier A, Ligne " + i + "\"Quantit�\" non-num�rique (=" + item_count + "), consid�r� comme z�ro"); 
				console.log("Error : QTY (col. 6) not numerical", rows[i]);
				item_count = 0;			
			}
			// ... et colonnes inutilis�s :
			// [2] Num�ro de mod�le
			// [3] ASIN
			// [7] Date d'expiration (produits p�rissables uniquement)
			// [8] Num�ro de lot
			
			/* 1.b) cr�ation de ligne "Carton" en fonction des lignes "Produit" */
			var container = groupsByContainer[container_id];
			if (container){
				//-- lignes suppl�mentaires (2e et suivantes) du m�me carton
				if (item_id != container.item_id) {
					container.item_id = "EAN MIXTE";
				}
				container.item_count += item_count;
			} else {
				//-- 1�re ligne d'un carton (tous cartons, homog�nes ou mixtes)
		...
		keys.sort(); // L'ordre de num�rotation est donc l'ordre des container_id 
		for (var i=0; i<keys.length; i++) {
			var container = groupsByContainer[keys[i]];
			this._containers.push(container);
			
			container.container_index = i+1; // c.� d. index dans l'ordre des container_id croissants
			container.container_count = keys.length;
			
			// op�. suppl�m. : pr�paration calcul de repartition_index et repartition_count
	...
	renderAllStickers : function(){
		...
		if (!isNaN(max_stickersPerPage)) {
			...
			GuiMessage.info("Nombre d'�tiquettes par page : " + max_stickersPerPage);
		} else {
			GuiMessage.warn("Erreur de template - nombre d'�tiquettes par page mal sp�cifi� (=" + max_stickersPerPage + "), donc mis par d�faut � 1"); 
		...
		for (var i=0; i<this._containers.length; i++) {
			...
			newLabel.populateStickerBarcode(".container_id", container.container_id); // remarque: type=code39, pas code 128 comme indiqu� ?!
...
/* *** Gestion donn�es compl�mentaires CSV "num�ro de palettes", dit "fichier P" *** */
var PalletContent = {
	_palletRows: [],
	...
	captureRows: function(rows){
		...
		if (rows[0].length < 2){
			/* check type fichier : test nombre colonnes, sur 1�re ligne seulement) */
			GuiMessage.error("Fichier P attendu avec 2 colonnes min. (num�ro de palette et EAN)");
			return -1;
		}
		if (rows[0].length >= 7){
			/* check confusion avec fichier A (� supposer que le fichier P reste plus petit !...) */
			GuiMessage.error("Fichier P avec trop de colonnes, confusion possible fichier A ? (" + rows[0].length + " colonnes trouv�es)");
			return -1;
		}

		var match_item_count;
		if (rows[0].length == 2){
			/* mode d�grad� "2 colonnes" (de fait, on remarque que la quantit� ne varie "jamais" avec les colis "non-mixtes") */
			GuiMessage.warn("Fichier P avec 2 colonnes (num�ro de palette et EAN) : mode simplifi�");
			match_item_count = false;
		} else {
			/* mode normal "3 colonnes" */
			GuiMessage.info("Fichier P avec au moins 3 colonnes (num�ro de palette, EAN et quantit�) : mode normal");
			match_item_count = true;			
		}
		GuiMessage.info("Fichier P Lignes lues dans : " + rows.length + " (header �ventuel compris)"); // header �ventuel compris
		
		for (var i=0; i<rows.length; i++) {
			var palletCsvRow = rows[i];
	
			/* 1) Mapping fichier CSV */
			var pallet_number = palletCsvRow[0]; // Num�ro de palette
			var item_id       = "" + palletCsvRow[1]; // EAN 
			var item_count    = (match_item_count ? palletCsvRow[2] : -1); // Quantit�
			...
			if (!isNaN(item_count)) {
				...
				GuiMessage.warn("Fichier P, ligne " + i + ": \"Quantit�\" non-num�rique (=" + item_count + ") : seul l'EAN sera utilis� sur cette ligne");
				item_count = -1;
			}
					
			if (item_asin ==  undefined && "EAN MIX" != item_id) {
				GuiMessage.warn("Fichier P, ligne " + i + " ean "+ item_id +" non trouv� dans ean-amazon");
			}
			
			/* 2) cr�ation de ligne "palette" */
	...
	resolvePalletIds: function(){
		...
		for (var j=0; j<AmzContainers._containers.length; j++){
			//var container = AmzContainers._containers[j]; inutilis�... pour le moment
		...
		for (var i=0; i<this._palletRows.length; i++){
			...
			for (var j=0; j<AmzContainers._containers.length; j++){
				...
				if (container_dispatch[j] >= 0) continue; // colis d�j� attribu�
		...
		for (var i=0; i<this._palletRows.length; i++){
			...
			if (attributed_container_index < 0) {
				GuiMessage.warn("Fichier P, Ligne "+ (i+1) +" n'a pas �t� utilis�e pour attribuer un num�ro de palette (ean=" + palletRow.item_id 
					+ (palletRow.item_count!=-1? ", qt�="+ palletRow.item_count : "") 
		...
		for (var j=0; j<AmzContainers._containers.length; j++){
			...
			if (attributed_palletRow_index >= 0) {
				...
				container.pallet_number = "";
				GuiMessage.warn("Num�ro de palette non-attribu� sur colis "+ container.container_index +"/"+ container.container_count +": "
				  + container.container_id +" (ean="+ container.item_id +", qt�="+ container.item_count +")");
			}
		}
...
function initShipToCombobox(){
	...
	Papa.parse(addressesUrl,{
		...
		...on est donc forc� d'utiliser "complete" pour charger les options */
		complete: function(results){
			...
			if (options.length == 0)
				GuiMessage.error("Fichier centre amazon vide � l'adresse : "+ addressesUrl);
			else if (!options[0].address.NOM || !options[0].address.L1 || !options[0].address.L2 || !options[0].address.L3 || !options[0].address.L4)
				GuiMessage.error("Fichier centre amazon non-conforme � l'adresse web : "+ addressesUrl);
			else
		...
		error: function(error, file){
			...
			GuiMessage.error("Fichier centre amazon Erreur de configuration des centres Amazon � l'adresse : '"+ addressesUrl + "'");
			GuiMessage.error("Fichier centre amazon (Si vous utilisez cette application en mode local, il est possible que votre navigateur ne supporte pas le chargement des fichiers locaux pour des raisons de s�curit�)") ;
			// remarque : le chargement de fichier local n'est pas support� par Chrome... mais Firefox le supporte
		}		
	});
}
	...
	req.onreadystatechange = function(event) {
		...
		if (this.readyState === XMLHttpRequest.DONE) {
			if (this.status === 200) {
				...
				GuiMessage.error("Fichier ean-amazon. Erreur: v�rfier que le fichier existe et qu'il est d�pos� au m�me endroit que index.html");
			}
		}
	};
...
/* handler S�lection du fichier A (g�n�r� sur le site Amazon) */
function handleFileASelect(evt) {	
	// -- Reset rendu
	EntryState.setFieldState("amazon-datafile", false);	
	setAppMode("test");
 	$("#printable-zone #result-zone").empty(); //m�me en cas d'�chec, pour plus de clart�
 	AmzContainers.clear();
 	
 	// -- Lecture fichier s�lectionn� 	
	...
	//GuiMessage.open();
	GuiMessage.info("Fichier A s�lectionn� : " + dataCvsFile.name + " ("+  dataCvsFile.size + "o)");
	
	Papa.parse(dataCvsFile, {
		header: false,        // -- Noms de colonnes non-fiables => 1�re ligne � sauter "manuellement"
		dynamicTyping: false, // -- ... certains codes num�riques ont des "leading zeroes"
		worker: false,        // -- pour un usage en "site local", les navigateurs consid�reraient que "worker : true" violerait la Same Origin Policy
		...
		complete: function(results, file){
			...
			GuiMessage.ok("�tiquettes trouv�es : " + containerCount);
			
			var resolvedCount = PalletContent.resolvePalletIds();
			if (resolvedCount>0) {
				GuiMessage.ok("Num�ros de palette attribu�s : " + resolvedCount + " (avec fichier P d�j� s�lectionn�)");
			}
			
			var pageCount = AmzContainers.renderAllStickers();
			GuiMessage.ok("Pages � imprimer : " + pageCount);
...
/* handler S�lection du fichier P (version CSV du fichier Excel utilis� au Packaging) */
function handleFilePSelect(evt) {	
	// -- Reset donn�es compl�mentaires
	...
	//GuiMessage.open();
	GuiMessage.info("Fichier P s�lectionn� : " + dataCvsFile.name + " ("+  dataCvsFile.size + "o)");
	
	Papa.parse(dataCvsFile, {
		header: false,        // -- Noms de colonnes non-fiables => 1�re ligne � sauter "manuellement"
		dynamicTyping: true,  // -- Pour le coup, on ignore compl�tement le probl�me des "leading zeroes", compte tenu du mode de g�n�ration de ce fichier-ci
		worker: false,        // -- pour un usage en "site local", les navigateurs consid�reraient que "worker : true" violerait la Same Origin Policy
		skipEmptyLines: true, // -- (notamment pour le saut de ligne en fin de fichier)
		encoding: "iso-8859-1",
		...
		complete: function(results, file){
			...
			GuiMessage.info("Fichier P, Nombre d'affectations de palette trouv�es : " + palletRowCount); // �gal au nombre de lignes ou inf�r. de 1 (pour le header)
			
			// refaire le rendu si des num�ros de palette ont pu �tre trouv�s
			var resolvedCount = PalletContent.resolvePalletIds();
			if (resolvedCount>0) {
				GuiMessage.ok("Num�ros de palette attribu�s : " + resolvedCount);
...
/*
arg "mode" parmi : 
	"test" : pas de vraies donn�es, affichage du template
	"real" : vraies donn�es dispo (fichier A), plus besoin du template
	"print" : on cache tout ce qui n'est pas �tiquettes
*/	
